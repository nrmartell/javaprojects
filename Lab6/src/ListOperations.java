...
public class ListOperations {
	...
	public static void main(String[] args) {
		...
		MyListOfInts tt = new MyListOfInts(t);
		tt = removeANumberFromMyList(t, 3);
	...
	*/
	...
	public static MyListOfInts reverseMyList (MyListOfInts m){
	    ...
	    // Since first is the new last, will make its restOfTheInts null.
...
//			m.restOfTheInts = m.restOfTheInts.restOfTheInts;
	...
	*/
	...
	int insertee, int position){
		...
		// Cannot go more than the length of list
		if(position < 0)
			return m;
		
		// index has been reached
		if(position == 0) {
			// Create a new list whose next is m
			MyListOfInts newList = new MyListOfInts(insertee);
			newList.restOfTheInts = m;
